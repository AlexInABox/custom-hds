    ...
    await fetch(history_url, {
      ...
      .then((body) => {
        try {
          ...
          showId = String($(".col.title").first().find("a").attr("href")).split(
            "/"
          )[2]; // "/title/80100172" -> "80100172"

          if (apiKey && showId) {
            //if apiKey is undefined, we don't have to fetch the cover image, also if showId is undefined, we don't have to fetch the cover image (this only applies to no-history accounts)
            console.log(
              "\x1b[31m",
              "[NETFLIX] Successfully fetched latest Netflix stream!"
            );
            ...
            console.log(
              "\x1b[31m",
              "[NETFLIX] Successfully fetched latest Netflix stream (without cover image)!"
            );
            return;
          }
        } catch (e) {
          console.log(
            "\x1b[31m",
            "[NETFLIX] Failed to fetch Netflix data, probably because the cookie expired."
          );
          return;
        }
      });
...
async function fetchDefaultImage(apiKey, showId, title, date) {
  try {
    showId = await fetch("https://www.netflix.com/title/" + showId).then(
      (res) => {
    ...
    console.log(
      "\x1b[31m",
      "[NETFLIX] Failed to fetch redirect URL, exiting..."
    );
  ...
  var fetch_url =
    "https://api.apilayer.com/unogs/title/details?netflix_id=" + showId;
  var headers = {
    apikey: apiKey,
  };
  ...
  console.log(
    "\x1b[31m",
    "[NETFLIX] Fetching Netflix cover image for " + showId + "..."
  );

  for (var i = 0; i < 5; i++) {
    //As of recently, the API is a bit unstable, so we try to fetch the image 5 times before giving up
    try {
      await fetch(fetch_url, {
        ...
        .then((body) => {
          if (body.message) {
            console.log(
              "\x1b[31m",
              "[NETFLIX] Failed to fetch Netflix cover image, API error: "
            );
          ...
          console.log(
            "\x1b[31m",
            "[NETFLIX] Successfully fetched Netflix cover image!"
          );
          ...
          encodeImageToBlurhash(large_image)
            ...
            .catch((error) => {
              console.error(
                "\x1b[31m",
                "[NETFLIX] I failed miserably gereating the blurhash. mb fam- " +
                  error
              );
              presence.patchNetflix(
                title,
      ...
      if (i == 4) {
        console.log(
          "\x1b[31m",
          "[NETFLIX] Failed to fetch Netflix cover image after 5 attempts, exiting..."
        );
      ...
      console.log(
        "\x1b[31m",
        "[NETFLIX] Something went wrong, retrying... (" + (i + 1) + " / 5)"
      );
