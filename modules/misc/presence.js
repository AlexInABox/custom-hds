...
class presence {
  ...
  patchHDSOxygenSaturation(oxygenSaturation) {
    console.log(
      "\x1b[36m",
      "[HDS] Received oxygen saturation: " + oxygenSaturation
    );
  ...
  patchNetflix(title, defaultImage, blurhash, date, showId) {
    ...
    if (
      netflix.lastWatched.title == title &&
      netflix.lastWatched.date == date &&
      netflix.lastWatched.showId == Number(showId)
    )
      return;
...
function patchPresence() {
  fs.writeFileSync(
    path.join(__dirname, "..", "..", "presence.json"),
    JSON.stringify(realPresence, null, 4)
  );
}
