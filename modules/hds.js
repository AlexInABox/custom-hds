...
function initializeServer() {
  ...
  app.post("/", (req, res) => {
    //auto export health - works in theory but auto export health as an app doesnt work as expected at times...
    ...
    try {
      if (
        convertDateToUnixTimestamp(
          metrics[0].data[metrics[0].data.length - 1].date
        ) < presence.getHDSHeartRateTimestamp()
      ) {
      ...
      console.log(
        "\x1b[36m",
        "[HDS] Received regular AutoExport data: " +
          metrics[0].data[metrics[0].data.length - 1].Max +
          "BPM"
      ); //min and max are the same when setting aggregation to seconds
      ...
      presence.patchHDSHeartRateWithTimestamp(
        Number(metrics[0].data[metrics[0].data.length - 1].Max),
        convertDateToUnixTimestamp(
          metrics[0].data[metrics[0].data.length - 1].date
        )
      );
      ...
      console.log(
        "\x1b[36m",
        "[HDS] Something went wrong when handling AutoExport data..."
      );
      console.log(metrics);
      res.sendStatus(500);
      return;
