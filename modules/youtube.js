...
class youtube {
  constructor(
    ...
    main(musicEnabled, videosEnabled, cookie);
    setInterval(() => {
      main(musicEnabled, videosEnabled, cookie);
    }, updateInterval * 1000); // 1000 ms = 1 second
  }
}
...
var yt;
async function main(musicEnabled, videosEnabled, cookie) {
  ...
  yt.session.on("auth-pending", (data) => {
    console.log(
      "\x1b[34m",
      `[YOUTUBE] Go to ${data.verification_url} in your browser and enter code ${data.user_code} to authenticate.`
    );
  });
  */
...
function patchPresenceMusic() {
  encodeImageToBlurhash(mThumbnail)
    .then((hash) => {
      console.log(
        "\x1b[31m",
        "[NETFLIX] Successfully generated blurhash: " + String(hash)
      );
      presence.patchYouTubeMusic(
        String(mTitle),
        String(mArtist),
        String(mUrl),
        String(mThumbnail),
    ...
    .catch((error) => {
      console.error(
        "\x1b[31m",
        "[NETFLIX] I failed miserably gereating the blurhash. mb fam- " + error
      );
      presence.patchYouTubeMusic(
        String(mTitle),
        String(mArtist),
...
async function fetchYTMusicData() {
  try {
    ...
    console.log(
      "\x1b[34m",
      `[YOUTUBE] Something went wrong trying to fetch the latest YTMusic Song: `
    );
    console.log(error);
  }
}
...
async function fetchYTVideoData() {
  try {
    ...
    console.log(
      "\x1b[34m",
      `[YOUTUBE] Something went wrong trying to fetch the latest YouTube Video: `
    );
    console.log(error);
  }
}
...
async function getLatestYTMusicSong() {
  const history = await yt.actions.execute("/browse", {
    client: "YTMUSIC",
    browseId: "FEmusic_history",
    parse: true,
  });
  //console.log(history.contents_memo.get("MusicResponsiveListItem")[0].title);
  return history;
}
...
function getLatestYTMusicArtist(history) {
  ...
  if (
    Array.isArray(
      history.contents_memo.get("MusicResponsiveListItem")[0].artists
    )
  ) {
    var length = history.contents_memo.get("MusicResponsiveListItem")[0].artists
      .length;
    for (let i = 0; i < length; i++) {
      if (i == 0) {
        artists = history.contents_memo.get("MusicResponsiveListItem")[0]
          .artists[i].name;
      } else {
        artists =
          artists +
          " & " +
          history.contents_memo.get("MusicResponsiveListItem")[0].artists[i]
            .name;
      }
    }
  } else {
    try {
      artists = history.contents_memo.get("MusicResponsiveListItem")[0].authors
        .name;
    } catch (error) {
      artists = "Unknown";
    }
...
function getLatestYTMusicURL(history) {
  return (
    ...
    history.contents_memo.get("MusicResponsiveListItem")[0].id
  );
}
...
function intialFetchOfDefaultThumbnail() {
  return new Promise(async (finish) => {
    const defaultResponse = await fetch(
      "https://i.ytimg.com/vi/vi/0/maxresdefault.jpg"
    ); //TODO: only fetch once (or once per iteration)
...
function isNotDefault(url) {
  //get the uint8Array buffer and compare it to the default buffer string ("lorem ipsum")
  return new Promise(async (res) => {
    const response = await fetch(url);
  ...
  return new Promise((res) => {
    if (ythistory.sections[0].contents[0].type == "ReelShelf") {
      ...
      videoURL =
        ...
        ythistory.sections[0].contents[1].id;
      ...
      videoURL =
        "https://www.youtube.com/watch?v=" +
        ythistory.sections[0].contents[0].id;
