...
function initializeServer(GEOAPIFY_API_KEY) {
  ...
  app.listen(port, () => {
    console.log(
      "\x1b[36m",
      "[Location] Location is now listening on port " + port
    );
  });

  app.post("/", async (req, res) => {
    console.log("\x1b[36m", "[Location] Received a location update request!");
    console.log("\x1b[36m", "[Location] " + JSON.stringify(req.body));
    ...
    try {
      await fetch(
        "https://api.geoapify.com/v1/geocode/reverse?lat=" +
          ...
          "&lon=" +
          req.body.lon +
          "&lang=en&limit=1&format=json&apiKey=" +
          GEOAPIFY_API_KEY,
        requestOptions
      )
        ...
        .then((result) => {
          ...
          fetch(
            "https://api.geoapify.com/v1/geocode/search?text=" +
              String(result.results[0].postcode) +
              ", " +
              String(result.results[0].district) +
              ", " +
              String(result.results[0].suburb) +
              ", " +
              String(result.results[0].city) +
              ", " +
              String(result.results[0].state) +
              ", " +
              String(result.results[0].country) +
              "&lang=en&limit=1&format=json&apiKey=" +
              GEOAPIFY_API_KEY,
            requestOptions
          )
            .then((response) => response.json())
            .then((result2) => {
              presence.patchLocation(
                Number(result2.results[0].lat.toFixed(2)),
                Number(result2.results[0].lon.toFixed(2)),
                result.results[0].district,
      ...
      console.log(
        "\x1b[35m",
        "[LIFE360] Failed to fetch location, due to an invalid API key. (Presumably)",
        e
      );
      return;
    }
